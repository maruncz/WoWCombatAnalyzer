...
QString LineParser::removequotes(QString s)
{
    ...
    {
        throw CombatLogParserException(QString("bad string format: %1").arg(s).toStdString());
    }
    s.remove('"');
    return s;
}
...
bool LineParser::parseBool(QString s)
{
    ...
    {
        throw CombatLogParserException(QString("unexpected bool value: %1").arg(s).toStdString());
    }
}
...
AuraType LineParser::parseAuraType(QString s)
{
    ...
    {
        throw CombatLogParserException(QString("unexpected aura type value: %1").arg(s).toStdString());
    }
}
...
PowerType LineParser::parsePowerType(QString s)
{
    ...
    if(val >= 26)
    {
        throw CombatLogParserException(QString("unexpected power type: %1").arg(val).toStdString());
    }
...
MissType LineParser::parseMissType(QString s)
{
    ...
    {
        throw CombatLogParserException(QString("unexpected miss type value: %1").arg(s).toStdString());
    }
}

template<typename T>
T LineParser::parseNumber(QString s, uint8_t base)
{
    bool ok{false};
    T val{0};
    ...
    else if constexpr (std::is_same_v<T, uint8_t>)
    {
        ...
        if(tmp > 255)
        {
            throw CombatLogParserException(QString("out of range: %1").arg(tmp).toStdString());
        }
    ...
    if(!ok)
    {
        throw CombatLogParserException(QString("error parsing number: %1, base: %2").arg(s).arg(base).toStdString());
    }
    return val;
}
